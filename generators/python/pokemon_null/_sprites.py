...
def _process_item_sprites(item_dir: str):
    ...
    output = directories.get_output_dir("pokemon_null/assets/items", True)
...
def _process_pokemon_sprite_images(sprite_dir: os.DirEntry, output_name: str,
    ...
    for sprite in _valid_names:
        ...
        if not os.path.isfile(target_palette):
            target_palette = os.path.join(sprite_dir.path, "..",
                                          f"normal{"f" if is_female_sprite else ""}.pal")
        ...
        if not os.path.isfile(target_palette):
            target_palette = os.path.join(sprite_dir.path, "..",
                                          f"shiny{"f" if is_female_sprite else ""}.pal")
...
def _process_pokemon_sprites(pokemon_dir: str):
    ...
    output = directories.get_output_dir("pokemon_null/assets/pokemon", True)
    directories.get_output_dir("pokemon_null/assets/pokemon/party", True)
    directories.get_output_dir("pokemon_null/assets/pokemon/normal", True)
    directories.get_output_dir("pokemon_null/assets/pokemon/shiny", True)
                    ...
                    for c in os.scandir(f.path):
                        if c.is_dir():
                            if p_dir.name == "tatsugiri" and c.name == "mega":
                                ...
                                if f.name == "curly":
                                    _process_pokemon_sprite_images(f, f"{p_dir.name}",
                                                                   output)
                                
                                _process_pokemon_sprite_images(c, f"{p_dir.name}-{f.name}_mega",
                                                               output)
                                
                            elif p_dir.name == "zygarde" and f.name == "complete" and c.name == "mega":
                                _process_pokemon_sprite_images(c, f"{p_dir.name}-{f.name}_mega",
                                                               output)
...
def process():
    ...
    root_dir = os.path.join(directories.get_external_dir("private"), "pokemon_null",
                            "graphics")
